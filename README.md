...
| `起動ブーストを設定 デューティ … 時間 …ms` | 200 / 50ms | 停止(または逆転)からの起動時だけ、目標速度より強い 200/255 を 50ms 与えて静止摩擦を越える(実測: 0msは起動不安定、50msで安定)。目標速度が 200 以上のときは何もしない |
...
* **左右の車輪は `モーター M1 … とモーター M2 …`(デュアルブロック)で指令してください。**
  ...
  遅れ、起動時に車体の向きが変わります。デュアルブロックは左右を同時にブーストします。
