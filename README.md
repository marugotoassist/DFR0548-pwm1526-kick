...
| `最低速度を設定 …` | 30 | 速度 1〜255 を 30〜255 に線形リマップ(デッドバンド補正)。**要調整**: 実測した「回転を維持できる最低速度」に置き換える |
...
* 起動ブースト中は `モーター` ブロックが指定時間(既定100ms)だけ待ちます。
  左右のモーターを続けて指令すると、右側の起動が 100ms 遅れます。
  気になる場合はブースト時間を短くするか、`起動ブーストを設定` の時間を 0 にして無効化してください。
